...
_Generated: 2026-07-03T21:34:45Z · commit: fd5ccba_
...
> F-A4 TRUST BOUNDARY STATUS: the old `agent`-owned config/LaunchAgent blocker is resolved. `openclawgw` cannot write the root-owned controls, and `agent` cannot replace them via directory-write attack. Phase 5 half-2 proved the proxy+pf wall, but it is not integrated because Blocker A remains: the `openclaw.json` proxy block breaks the Gmail reader path (`Proxy response (400) !== 200 when HTTP Tunneling`), so Gmail-via-broker must not inherit the gateway egress proxy; scope the proxy to exclude the Gmail path. Blocker B is RESOLVED and was misdiagnosed: Gmail broker failure was not OAuth; `/var/run/agent-os` was wiped by reboot and the broker crashed before reaching Google.
...
**FLAG:** Egress allowlist is IN BUILD under F-A4. Gateway identity/config custody is locked and the permanent root/operator-owned CONNECT proxy plus pf backstop were proven on 2026-06-28, but they are not integrated until the Gmail path is excluded from inheriting the gateway proxy, pf is reloaded, and the wall is re-run through acceptance 7a-7e. Egress alone does not unlock sensitive data.
...
> F-A4  Egress allowlist                  ← IN BUILD (gateway re-home/tamper-lock proven; Phase 5 proxy half-1 and half-2 proven; Gmail proxy-path exclusion pending)
...
>
> **IMMEDIATE NEXT: Resolve F-A4 Blocker A only — re-add proxy block with Gmail-path exclusion, then reload pf and re-run acceptance 7a-7e.**
> Current safe baseline: Gmail is working, pf is off, and there is no `openclaw.json` proxy block. Do not revisit broker OAuth for this outage; the 2026-07-03 root cause was missing `/var/run/agent-os`, now fixed with boot persistence. The next mutating step is to scope the gateway proxy so Gmail-via-broker does not inherit it, then reload pf and prove the full wall again.
>
> **F-A4 BLOCKERS:**
> A — Proxy block in `openclaw.json` breaks the Gmail reader path (`Proxy response (400) !== 200 when HTTP Tunneling`). Gmail-via-broker must not inherit the gateway egress proxy. Fix: scope the proxy to exclude the Gmail path.
...
>
> **NEXT SESSION ORDER:**
> (1) Re-add proxy block with Gmail-path exclusion. (2) Reload pf, re-run full acceptance 7a-7e. (3) Tighten DNS rule, currently port-53 to ANY dest, to the system resolver only. (4) Install pf persistence LaunchDaemon + reboot test. (5) Install add-allow operator helper.
...
- 2026-06-28 | Gmail-via-broker path must be excluded from the gateway egress proxy | Inheriting the proxy breaks the reader path (Proxy 400 !== 200, HTTP Tunneling).

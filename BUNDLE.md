...
_Generated: 2026-07-03T21:01:56Z · commit: a3d31c3_
...
Current drift state: agent-os at this commit; ~/.openclaw has been re-homed under the F-A4 cutover. Gateway now runs as `openclawgw` (UID 555) under root LaunchDaemon `/Library/LaunchDaemons/ai.openclaw.gateway.plist`; the three-tier tamper lock is re-asserted and proven after the 2026-06-21 recovery. F-A4 Phase 5 half-2 was built and proven on 2026-06-28, then left not integrated: intended close baseline was pf disabled, proxy block removed from `openclaw.json`, gateway restarted, and Lloyd working for non-Gmail tasks. This was recorded late on 2026-07-03 after a week-long SSH lockout and an intervening ungraceful power loss; live pf status and `openclaw.json` proxy-block state have NOT been re-verified since the power loss and must be confirmed before the next mutating step.
...
> F-A4 IN BUILD: gateway re-home and tamper-lock are EXECUTED and PROVEN; Phase 5 half-1 and half-2 are BUILT AND PROVEN, but not integrated. Half-2 proof on 2026-06-28 wired the locked `openclaw.json` proxy block, loaded the pf anchor, proved `openclawgw` direct egress to `example.com` and `chatgpt.com` timed out, and proved web_search through the proxy. Intended close baseline was reverted to pf disabled, proxy block removed, gateway restarted, and Lloyd working for non-Gmail tasks. Because an ungraceful power loss occurred before this late 2026-07-03 record, live pf status and proxy-block state must be re-verified before any mutating step. Native container sandbox / default-deny was DISPROVED as the F-A4 answer in F-A4-1/1b and reverted: web_search runs in the host gateway process outside the container path, and enabling the Docker sandbox broke F-A1/F-A2/F-A3 load-bearing paths.
...
> F-A4 TRUST BOUNDARY STATUS: the old `agent`-owned config/LaunchAgent blocker is resolved. `openclawgw` cannot write the root-owned controls, and `agent` cannot replace them via directory-write attack. Phase 5 half-2 proved the proxy+pf wall, but it is not integrated because two independent blockers remain: (A) the `openclaw.json` proxy block breaks the Gmail reader path (`Proxy response (400) !== 200 when HTTP Tunneling`), so Gmail-via-broker must not inherit the gateway egress proxy; scope the proxy to exclude the Gmail path. (B) Gmail broker `search_threads` fails with `gmail_error` even with pf off and the proxy block removed; gmail-watcher has been `stopped` since 2026-06-21; broker stderr logging is too thin; Gmail API is operational; leading hypothesis is expired/revoked OAuth token in broker gog-home/secrets.
...
**FLAG:** Egress allowlist is IN BUILD under F-A4. Gateway identity/config custody is locked and the permanent root/operator-owned CONNECT proxy plus pf backstop were proven on 2026-06-28, but they are not integrated until Gmail broker auth is fixed, the Gmail path is excluded from inheriting the gateway proxy, pf/proxy state is re-verified after the power loss, and the wall is re-run through acceptance. Egress alone does not unlock sensitive data.
...
> F-A4  Egress allowlist                  ← IN BUILD (gateway re-home/tamper-lock proven; Phase 5 proxy half-1 and half-2 proven; Gmail integration + broker auth pending)
...
>
> **IMMEDIATE NEXT: Diagnose Gmail broker auth (independent of F-A4).**
> Goal: inspect gmailbroker gog-home / secrets / bin, capture the raw Gmail API error, and determine whether the failure is `invalid_grant`/401 versus another broker/runtime issue. Leading hypothesis is expired or revoked OAuth token in broker gog-home/secrets; likely fix is broker re-auth. Before any mutating step, re-verify live pf status and `openclaw.json` proxy-block state because an ungraceful power loss occurred after the 2026-06-28 proof and before this 2026-07-03 late record.
>
> **F-A4 BLOCKERS:**
...
> B — Gmail broker fails `search_threads` with `gmail_error` even with pf off and the proxy block removed. This is pre-existing and independent of the wall. `gmail-watcher` has been `stopped` since 2026-06-21. Broker `stderr.log` has no runtime error detail, so logging is too thin and must be fixed. Gmail API is confirmed operational. Leading hypothesis: expired/revoked OAuth token in broker gog-home/secrets.
>
> **NEXT SESSION ORDER:**
> (1) Fix Gmail broker auth. (2) Re-add proxy block with Gmail-path exclusion. (3) Reload pf, re-run full acceptance 7a-7e. (4) Tighten DNS rule, currently port-53 to ANY dest, to the system resolver only. (5) Install pf persistence LaunchDaemon + reboot test. (6) Install add-allow operator helper.
